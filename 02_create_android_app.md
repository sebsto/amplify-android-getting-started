...
- [Kotlin](https://developer.android.com/kotlin) - I choose to use the Kotlin programng language for thsi tutorial, because it allows to reduce much of the boilerplate code and it is designed with type safety in mind. 
...
Under **Phone and Tabler**, select **Basic Activity** and click **Next**
...
3. Create a View to hold an individual cell in the list 
4. Modify the `MainActivity` to display a list of cells 

## Delete the files we do not need

Under `res/layout` and `java/com.example.androidgettingstarted` delete the four `**fragment**` files (select 4 files, right click and choose **Delete** from the contextual menu):
...
Individual cells in a scrolling list are called [RecyclerView](https://developer.android.com/guide/topics/ui/layout/recyclerview), as the View can be recycled when the user scrolls up and down, and the view is not visible on the screen anymore. 
...
To create the layout file, right click `res/layout` and select **New** -> **Layout Resource File**. Type  **content_node** as name and leave all the other values, as we will directly edit the XML file.

![android studio individual cell](img/02_60.png)

Open the **Code** view for the newly created file.

![android studio edit content_node](img/02_70.png)

In the Code view of the new file just created (`content_node.xml`), replace the code generated by pasting the code below:
...
- a layout xml file that describe the layout of component on a cell representing a `Note`. It display the image, the note title, and the note description.

- a support Kotlin class. It receives a `Note` data object at creation time and assign individual values to their coressponding views (image, title and description)
...
- the main layout is a [RecyclerView](https://developer.android.com/guide/topics/ui/layout/recyclerview) that manage the list of individual cells we created previoulsy 
